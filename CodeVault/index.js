...
app.get("/api/account", requireAuth, async (req, res) => {
  try {
    ...
    const resp = await notion.databases.query({
      database_id: teamMembersDatabaseId,
      filter: { property: "Name", title: { equals: String(req.session.username || "") } }
    });
    if (!resp.results || resp.results.length === 0) {
    ...
    const phoneOf = (prop) => prop?.phone_number || null;
    const emailOf = (prop) => prop?.email || null;
    const numberOf = (prop) => (typeof prop?.number === "number" ? prop.number : null);
    const selectOf = (prop) => (prop?.select?.name || null);
    const multiOf = (prop) => {
      if (prop?.multi_select?.length) return prop.multi_select.map(x => x?.name).filter(Boolean);
      if (prop?.relation?.length) return prop.relation.map(r => r?.id).filter(Boolean);
      return [];
    };

    const name         = rich(p["Name"]) || String(req.session.username || "");
    const department   = selectOf(p["Department"]);
    const position     = selectOf(p["Position"]);
    const email        = emailOf(p["Email"]);
    const phone        = phoneOf(p["Phone"]);
    const employeeCode = rich(p["Employee Code"]) ?? numberOf(p["Employee Code"]);
    const allowedPages = multiOf(p["Allowed Pages"] || p["Allowed pages"]);

    res.set("Cache-Control", "no-store");
    return res.json({
      name, department, position, email, phone, employeeCode, allowedPages
    });
  } catch (e) {
    console.error("GET /api/account error:", e.body || e)
