// CodeVault/index.js — Express app for Vercel/Node and local use via server.js
...
// Notion token (يدعم أكثر من اسم متغير للراحة)
const NOTION_TOKEN =
...
// DB IDs (نفس الأسماء الحالية عندك في Vercel)
...
// permissions middlewares (خفيفة: فقط تتأكد من تسجيل الدخول)
...
/* -------------------- Login / Logout -------------------- */
// ⚠️ تمت إزالة أي هاندلر قديم/فاضي لـ /api/login كان يعلّق الطلب.
app.post("/api/login", async (req, res) => {
  try {
    ...
    if (!teamMembersDatabaseId) {
      return res
        .status(500)
        .json({ error: "Team_Members DB not configured" });
    }
    ...
    const passVal =
      typeof passProp?.number === "number" ? passProp.number : null;
  ...
  async (_req, res) => {
    try {
      if (!componentsDatabaseId) {
        return res
          .status(500)
          .json({ error: "Products_Database not configured" });
      }
      const all = [];
      let start = undefined;
      let hasMore = true;
  ...
  async (req, res) => {
    try {
      if (!ordersDatabaseId || !teamMembersDatabaseId) {
        return res
          .status(500)
          .json({ success: false, message: "DB IDs not configured" });
      }
      ...
      req.session.recentOrders = (req.session.recentOrders || [])
        .concat(
          creations.map((c) => ({
            id: c.orderPageId,
            reason,
...
app.get(
  ...
  async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch requested orders" });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const sample = await notion.pages.retrieve({
        page_id: orderIds[0],
      });
    ...
    try {
      ...
      while (hasMore) {
        ...
        for (const page of r.results) {
          ...
          const reason =
            props.Reason?.title?.[0]?.plain_text || "No Reason";
          const status = statusProp
            ? props[statusProp]?.select?.name || ""
            : "";
          items.push({
            id: page.id,
            productName,
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch assigned orders" });
    }
  }
);
    ...
    try {
      ...
      if (!availableProp) {
        return res.status(400).json({
          error:
            'Please add a Number property "Available Quantity" to Orders DB.',
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (Number.isNaN(availNum) || availNum < 0) {
        return res.status(400).json({
          error: "available must be a non-negative number",
        });
      }
      const availableProp = await detectAvailableQtyPropName();
      if (!availableProp) {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!statusProp) {
        return res.status(400).json({
          error: 'Please add a Select property "Status" to Orders DB.',
        });
      }
      await Promise.all(
        orderIds.map((id) =>
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const colRemW =
        pageInnerWidth - colNameW - colReqW - colAvailW;
      ...
      const drawHead = () => {
        ...
        doc.save();
        doc
          .rect(doc.page.margins.left, y, pageInnerWidth, h)
          .fill("#F3F4F6");
      ...
      items.forEach((it) => {
        ...
        doc
          ...
          .lineTo(
            doc.page.margins.left + pageInnerWidth,
            y + h
          )
          .strokeColor("#EEE")
          .lineWidth(1)
  ...
  async (req, res) => {
    try {
      ...
      const schoolName =
        ...
        (Array.isArray(schoolProp?.title) &&
          schoolProp.title[0]?.plain_text) ||
        null;
      if (!schoolName)
        return res
          .status(404)
          .json({ error: "Could not determine school name." });
    ...
    try {
      ...
      (all || []).forEach((it) => {
        ...
        if (!groupsMap.has(key))
          groupsMap.set(key, { name, color, items: [] });
      ...
      const fname = `Stocktaking-${new Date()
        .toISOString()
        .slice(0, 10)}.pdf`;
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
      ...
      const gap = 10,
        colKitW = 120,
        colQtyW = 90,
        colNameW = pageInnerWidth - colKitW - colQtyW - gap * 2;
      ...
      const drawTableHead = (pal) => {
        ...
        const lastX =
          doc.page.margins.left + colNameW + gap + colKitW + gap;
        doc.text(
          "In Stock",
          lastX,
          y + 5,
          { width: colQtyW - 10, align: "right" }
        );
        doc.restore();
        doc.moveDown(1.2);
      };
      ...
      const ensureGroupStartSpace = () =>
        ensureSpace(22 + 20 + 18);
  ...
  try {
    ...
    if (typeof password !== "undefined") {
      ...
      if (Number.isNaN(n))
        return res
          .status(400)
          .json({ error: "Password must be a number." });
    ...
    try {
      if (!fundsDatabaseId) {
        return res.status(500).json({
          configured: false,
          error: "Funds DB ID not configured",
        });
      }
      ...
      res.status(500).json({
        configured: false,
        error: e.message || "Cannot access funds database",
      });
    }
  }
);
  ...
  try {
    ...
    for (const exp of expenses) {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "All expense fields are required and cost must be positive",
        });
      }
    }
    const userQ = await notion.databases.query({
                ...
                rich_text: [
                  {
                    type: "text",
                    text: {
                      content: `Receipt: ${screenshotName} (${
                        screenshotType || "type?"
                      }${
                        screenshotSize
