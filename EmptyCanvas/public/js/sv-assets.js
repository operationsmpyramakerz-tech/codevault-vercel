...
(function () {
  ...
  // ★ NEW: add S.V Comment column + send button
  function renderItemsTable(items) {
    ...
    const rows = items.map((it, i) => {
      ...
      const qty  = it.qty ?? 1;
      const note = it.note || "";
      const comment = it["S.V Comment"] || "";
      const files = Array.isArray(it.files) ? it.files : [];
      const filesHtml = files.map(f => `<span class="chip"><i data-feather="paperclip"></i> ${f.name || 'file'}</span>`).join(" ");
        ...
        <tr data-id="${id}">
          ...
          <td style="min-width:220px;">
            <div style="display:flex;gap:6px;align-items:center;">
              <input type="text" value="${comment || ''}" placeholder="S.V Comment" class="sv-comment-input" style="flex:1;padding:6px 8px;border:1px solid #E5E7EB;border-radius:6px;"/>
              <button class="btn btn-sm btn-primary sv-comment-send">Send</button>
            </div>
          </td>
        </tr>`;
    }).join("");
    ...
    return `
      <div class="table-responsive">
        <table class="table">
          <thead>
            <tr>
              <th>#</th><th>Product</th><th>Qty</th><th>Note</th><th>Files</th><th>S.V Comment</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
