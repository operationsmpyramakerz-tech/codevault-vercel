...
app.post(
  ...
  async (req, res) => {
    try {
      const databaseId = process.env.Damaged_Assets; // ID جدول Damaged Assets
      if (!databaseId) {
        return res.status(500).json({ success: false, error: 'Damaged_Assets env is missing.' });
      }
      ...
      // جِب صفحة عضو الفريق (اليوزر الحالي) علشان نحطها في Teams Members relation
      // على نفس طريقة مشروعك — لو عندك هيلبر جاهز استعمله بدل السطرين دول:
      const userEmail = req.user?.email || req.session?.user?.email;
      const teamMemberPageId = await getTeamMemberPageIdByEmail(userEmail); // لازم تكون موجودة عندك. إن لم توجد، رجّع null وسنُهمل العمود.

      const toRich = (s) => [{ type: 'text', text: { content: String(s || '') } }];

      const results = [];
      for (const item of items) {
        const props = {
          // Title: Description of issue
          'Description of issue': { title: toRich(item.title) },
          // Text: Issue Reason
          'Issue Reason': { rich_text: toRich(item.reason || '') },
          // Relation: Products
          ...(item.product?.id
            ? { 'Products': { relation: [{ id: String(item.product.id) }] } }
            : {}),
          // Relation: Teams Members (لو قدرنا نجيبه)
          ...(teamMemberPageId
            ? { 'Teams Members': { relation: [{ id: teamMemberPageId }] } }
            : {}),
        };

        // Files & media (اختياري): لو عندك URLs خارجية للملفات
        if (Array.isArray(item.files) && item.files.length && item.files[0].url) {
          props['Files & media'] = {
      ...
      console.error('POST /api/damaged-assets failed:', err.body || err);
